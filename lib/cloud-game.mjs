...
import { askJev, emojiFor, normalize, validateGuess } from './judge.mjs';
...
export function createCloudHandler({ key = process.env.TYPESAFE_API_KEY, model = process.env.JEV_MODEL || 'jev-1.13.0', secret = process.env.SESSION_SECRET || key, judge, now = Date.now } = {}) {
